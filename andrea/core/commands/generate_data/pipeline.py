...
TF_LIST_RELATIVE_PATH = "extras/tf_list.txt"
GENE_UNIVERSE_RELATIVE_PATH = "truth/gene_universe.txt"
...
def _dataset_manifest_payload(
    *,
    ...
    if not isinstance(extras, dict) or extras.get("tf_list") != TF_LIST_RELATIVE_PATH:
        raise ValueError(
            "simulator-output-manifest extras.tf_list must be "
            f"{TF_LIST_RELATIVE_PATH!r}"
        )
    dataset_spec = {
        "schema_version": "1.0",
        "id": dataset_id,
...
def _ground_truth_manifest_payload(
    *,
    ...
    if gene_universe != GENE_UNIVERSE_RELATIVE_PATH:
        raise ValueError(
            "simulator-output-manifest truth.gene_universe must be "
            f"{GENE_UNIVERSE_RELATIVE_PATH!r}"
        )
    return {
        "schema_version": "1.0",
        ...
        "candidate_space": {
            "sources": TF_LIST_RELATIVE_PATH,
            "targets": GENE_UNIVERSE_RELATIVE_PATH,
...
def _execute_simulation_task(
    *,
    ...
    if not isinstance(raw_extras, dict):
        raise ValueError(
            f"simulator-output-manifest[{dataset_id}].extras must be an object"
        )
    dataset_fingerprint = fingerprint_dataset_content(
        expression_path=stage_dir / "expression.tsv",
        extras={
            key: stage_dir / value
            for key, value in raw_extras.items()
            if key in _NORMALIZED_EXTRA_FILENAMES and isinstance(value, str)
        },
    )
