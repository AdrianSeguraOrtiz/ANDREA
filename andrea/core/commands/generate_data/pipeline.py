...
def _read_expression_gene_universe(expression_path: Path) -> list[str]:
    if not expression_path.exists() or not expression_path.is_file():
        raise ValueError(f"Expression matrix not found: {expression_path}")

    genes: list[str] = []
    seen: set[str] = set()
    with expression_path.open("r", encoding="utf-8", newline="") as fh:
        reader = csv.reader(fh, delimiter="\t")
        header = next(reader, None)
        if header is None or len(header) < 2:
            raise ValueError(
                f"Expression matrix must contain a gene column and at least one sample/cell column: {expression_path}"
            )
        for line_no, row in enumerate(reader, start=2):
            ...
            if not gene:
                raise ValueError(
                    f"Expression matrix has an empty gene identifier at line {line_no}: {expression_path}"
                )
            if gene in seen:
                continue
            seen.add(gene)
            genes.append(gene)
    if not genes:
        raise ValueError(f"Expression matrix contains no genes: {expression_path}")
    return genes


def _write_gene_universe_file(*, expression_path: Path, destination: Path) -> None:
    genes = _read_expression_gene_universe(expression_path)
...
def _copy_dataset_from_stage(
    *,
    ...
    _write_gene_universe_file(
        expression_path=dataset_dir / "expression.tsv",
        destination=dataset_dir / "truth" / "gene_universe.txt",
    )
