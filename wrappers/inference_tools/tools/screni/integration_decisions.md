# screni Integration Decisions

## Integration Status

- Phase executed: Phase 3 complete.
- Wrapper implementation: `wrappers/inference_tools/tools/screni/run_tool.R`.
- Dockerfile: `wrappers/inference_tools/tools/screni/Dockerfile`.
- Smoketest config: `wrappers/inference_tools/tests/smoketest_configs/screni.json`.
- Smoketest fixtures: `wrappers/inference_tools/tests/fixtures/screni/expression.tsv`, `wrappers/inference_tools/tests/fixtures/screni/groups.tsv`.
...
- Final Phase 3 validation:
  - `make validate-toolspecs ARGS="--tool screni"` passed.
  - `make validate-input-specs` passed for all 16 inference input specs, including relevant `expression_matrix` and `groups`.
  - `make validate-smoketest-configs ARGS="--tool screni"` passed.
- Final Phase 3 smoke command: `make run-tool-smoketests ARGS="--tool screni --threads 2 --timeout 2400 --show-output-lines 20"`.
...
- Upstream repo: `wrappers/inference_tools/tools/screni/repo/ScReNI`
- Local paper PDF: `wrappers/inference_tools/tools/screni/papers/ScReNI.pdf`
- Extracted paper text: `wrappers/inference_tools/tools/screni/papers/ScReNI.txt`
...
| `id` | `screni` | Tool folder and catalog path use `screni` | Project scaffold; certain. |
| `name` | `ScReNI (kScReNI)` | README title uses ScReNI; paper names kScReNI as scRNA-only variant (`README.md:4-13`, `papers/ScReNI.txt:149-177`) | Distinguishes selected public variant; certain. |
...
| `docker_image` | `adriansegura99/inference-tools_screni:1.0.0` | Existing inference-tool image naming convention | Project convention; local smoketest image `screni-smoketest:local` built successfully in Phase 2. |
...
| `artifacts_aux` | `screni.log`, `raw/kscreni_networks.rds` | kScReNI returns a raw per-cell matrix list; upstream prints Seurat/GENIE3 diagnostics | Implemented and validated by smoketest. |
...
- Auxiliary artifacts: writes `screni.log` and `raw/kscreni_networks.rds`.
...
- Fixture: `wrappers/inference_tools/tests/fixtures/screni/expression.tsv` has 15 cells and 15 variable non-negative genes so Seurat's default neighbor dimensions are available; `groups.tsv` maps the same 15 cells to two groups.
- Config: `nfeatures=15`, `knn=2`; variants cover `cell_native` and `group_aggregated`.
- Final command: `make run-tool-smoketests ARGS="--tool screni --threads 2 --timeout 2400 --show-output-lines 20"`.
- Result: passed.
- Output checks: each variant produced `network.csv` with 3081 positive non-self rows, final `progress.json` status `completed`, and required auxiliary artifacts `screni.log` plus `raw/kscreni_networks.rds`.
...
- Failure: both `screni__01` and `screni__02/upstream_cell_native` failed in `Seurat::RunPCA()` with `max(nu, nv) must be strictly less than min(nrow(A), ncol(A))` on a 137 gene x 50 cell expression matrix.
...
- Verification: rebuilt `adriansegura99/inference-tools_screni:1.0.0` and reran the GUI `screni__01` input directly in Docker; the run completed and wrote 610353 positive non-self cell-context edges. The same output aggregates with the GUI `groups.tsv` into 95328 group-context edges across 7 groups.
